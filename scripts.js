...
const HEIGHT = 600;
const MARGINS = {top: 0, bottom: 700, left: 0, right: 125};
const ROSEWIDTH = 350;
...
//						updateGlassFill(ext_svg.martiniGlass, d.spirit);
		...
		function makeRoseCharts(countryData)
		{
			...
			let regionAbbr = {
				...
				"Eastern Asia": "Asia",
				"Middle East and N Africa": "M. East & N. Africa"
			};
			...
			for (var i = 0; i < regionsByAlcohol3.length; i++) {
				var temp = {Region: regionAbbr[regionsByAlcohol3[i][0]] || regionsByAlcohol3[i][0], Wine_PerCapita: +regionsByAlcohol3[i][1].Wine_PerCapita, Beer_PerCapita: +regionsByAlcohol3[i][1].Beer_PerCapita, Spirit_PerCapita: +regionsByAlcohol3[i][1].Spirit_PerCapita};
			...
			for (var i = 0; i < easternEurope.length; i++) {
				var temp = {Country: countryData[easternEurope[i].Country] || easternEurope[i].Country, Wine_PerCapita: +easternEurope[i].Wine_PerCapita, Beer_PerCapita: +easternEurope[i].Beer_PerCapita, Spirit_PerCapita: +easternEurope[i].Spirit_PerCapita};
				easternEurope2[i] = temp;
			}
			easternEurope2.columns = ["Country", "Wine_PerCapita", "Beer_PerCapita", "Spirit_PerCapita"]
			roseChart("Eastern Europe", data, easternEurope2, 300, 180, false, true, 700);

			var subAfrica = countryAlcohol2[1][1];
			var subAfrica2 = [];
			for (var i = 0; i < subAfrica.length; i++) {						
				var temp = {Country: countryData[subAfrica[i].Country] || subAfrica[i].Country, Wine_PerCapita: +subAfrica[i].Wine_PerCapita, Beer_PerCapita: +subAfrica[i].Beer_PerCapita, Spirit_PerCapita: +subAfrica[i].Spirit_PerCapita};
				subAfrica2[i] = temp;
			}
			subAfrica2.columns = ["Country", "Wine_PerCapita", "Beer_PerCapita", "Spirit_PerCapita"]
			roseChart("Sub Sahara Africa", data, subAfrica2, 600, 180, false, true, 500);

			var southAmerica = countryAlcohol2[2][1];
			var southAmerica2 = [];
			for (var i = 0; i < southAmerica.length; i++) {
				var temp = {Country: countryData[southAmerica[i].Country] || southAmerica[i].Country, Wine_PerCapita: +southAmerica[i].Wine_PerCapita, Beer_PerCapita: +southAmerica[i].Beer_PerCapita, Spirit_PerCapita: +southAmerica[i].Spirit_PerCapita};
				southAmerica2[i] = temp;
			}
			southAmerica2.columns = ["Country", "Wine_PerCapita", "Beer_PerCapita", "Spirit_PerCapita"]
			roseChart("South America", data, southAmerica2,900, 180, false, true, 500);

			var aussie = countryAlcohol2[3][1];
			var aussie2 = [];
			for (var i = 0; i < aussie.length; i++) {
				var temp = {Country: countryData[aussie[i].Country] || aussie[i].Country, Wine_PerCapita: +aussie[i].Wine_PerCapita, Beer_PerCapita: +aussie[i].Beer_PerCapita, Spirit_PerCapita: +aussie[i].Spirit_PerCapita};
				aussie2[i] = temp;
			}
			aussie2.columns = ["Country", "Wine_PerCapita", "Beer_PerCapita", "Spirit_PerCapita"]
			roseChart("Australia and NZ", data, aussie2, 1200, 180, false, true, 600);

			var westernEurope = countryAlcohol2[4][1];
			var westernEurope2 = [];
			for (var i = 0; i < westernEurope.length; i++) {
				var temp = {Country: countryData[westernEurope[i].Country] || westernEurope[i].Country, Wine_PerCapita: +westernEurope[i].Wine_PerCapita, Beer_PerCapita: +westernEurope[i].Beer_PerCapita, Spirit_PerCapita: +westernEurope[i].Spirit_PerCapita};
				westernEurope2[i] = temp;
			}
			westernEurope2.columns = ["Country", "Wine_PerCapita", "Beer_PerCapita", "Spirit_PerCapita"]
			roseChart("Western Europe", data, westernEurope2, 0, 500, false, true, 600);

			var middleEast = countryAlcohol2[5][1];
			var middleEast2 = [];
			for (var i = 0; i < middleEast.length; i++) {
				var temp = {Country: countryData[middleEast[i].Country] || middleEast[i].Country, Wine_PerCapita: +middleEast[i].Wine_PerCapita, Beer_PerCapita: +middleEast[i].Beer_PerCapita, Spirit_PerCapita: +middleEast[i].Spirit_PerCapita};
				middleEast2[i] = temp;
			}
			middleEast2.columns = ["Country", "Wine_PerCapita", "Beer_PerCapita", "Spirit_PerCapita"]
			roseChart("Middle East", data, middleEast2, 300, 500, false, true, 200);

			var seAsia = countryAlcohol2[6][1];
			var seAsia2 = [];
			for (var i = 0; i < seAsia.length; i++) {
				var temp = {Country: countryData[seAsia[i].Country] || seAsia[i].Country, Wine_PerCapita: +seAsia[i].Wine_PerCapita, Beer_PerCapita: +seAsia[i].Beer_PerCapita, Spirit_PerCapita: +seAsia[i].Spirit_PerCapita};
				seAsia2[i] = temp;
			}
			seAsia2.columns = ["Country", "Wine_PerCapita", "Beer_PerCapita", "Spirit_PerCapita"]
			roseChart("South East Asia", data, seAsia2, 600, 500, false, true, 400);

			var northAmerica = countryAlcohol2[7][1];
			var northAmerica2 = [];
			for (var i = 0; i < northAmerica.length; i++) {
				var temp = {Country: countryData[northAmerica[i].Country] || northAmerica[i].Country, Wine_PerCapita: +northAmerica[i].Wine_PerCapita, Beer_PerCapita: +northAmerica[i].Beer_PerCapita, Spirit_PerCapita: +northAmerica[i].Spirit_PerCapita};
				northAmerica2[i] = temp;
			}
			northAmerica2.columns = ["Country", "Wine_PerCapita", "Beer_PerCapita", "Spirit_PerCapita"]
			roseChart("North America", data, northAmerica2, 1200, 500, false, true, 600);

			var eastAsia = countryAlcohol2[8][1];
			var eastAsia2 = [];
			for (var i = 0; i < eastAsia.length; i++) {
				var temp = {Country: countryData[eastAsia[i].Country] || eastAsia[i].Country, Wine_PerCapita: +eastAsia[i].Wine_PerCapita, Beer_PerCapita: +eastAsia[i].Beer_PerCapita, Spirit_PerCapita: +eastAsia[i].Spirit_PerCapita};
				eastAsia2[i] = temp;
			}
			eastAsia2.columns = ["Country", "Wine_PerCapita", "Beer_PerCapita", "Spirit_PerCapita"]
			roseChart("Eastern Asia", data, eastAsia2, 900, 500, false, true, 500);
		}

		function roseChart(title, dataset, customizedData, leftOffset, topOffset, legend, country, maxAmount) {
			...
			g.append("g")
				...
				//Tooltips!
				.on('mousemove',function(d){
					div.html('<span class="title">' + (country ? d.data.Country : d.data.Region) + "</span></br> Wine: " + d.data.Wine_PerCapita +  "</br>Spirits: " + d.data.Spirit_PerCapita + "</br> Beer: " +d.data.Beer_PerCapita)
						.style("opacity", 1)
						.style("left", (d3.event.pageX) - div.node().clientWidth/2 + "px")
						.style("top", (d3.event.pageY - div.node().clientHeight - 10) + "px");
				})
